...
export function apply(ctx, config = {}, internals = {}) {
  try {
    ...
    // 非 loopback 页面也启用 Host 设置文档（默认开，clientHostTrust: false 关闭）
    ...
    let proxy = null;
    let logViewer = null;
    let dshAuthStop = null;
    const disposers = [];

    // 代持 DSH 内置 BrowserAuth：同进程内用 launch-token 换取 DSH cookie，
    // 注入所有转发请求，浏览器无需再面对 DSH 的 token/cookie。
    const dshAuth = new DshAuthProvider(ctx.connection, { host: '127.0.0.1', port: dshPort });
    dshAuth.log = logger;
    if (dshAuth.available()) {
      dshAuthStop = dshAuth.start();
      logger.info('dsh-web-pass: DSH 内置认证代持已启用（connection.authenticatedUrl）');
    } else {
    ...
    const getStatus = async () => {
      ...
      return { proxyRunning: proxy !== null, proxyPort, lanUrl, lanQr: null, dshPort, logViewerPort: logViewer?.port ?? null, dshAuthHolding: !!dshAuth.cookie };
    };
    ...
    void createGateProxy({
      ...
      injectHtml: DEFAULT_INJECT, auth, dshAuth,
    ...
    try {
      ctx.effect(() => async () => {
        if (dshAuthStop) { try { dshAuthStop(); } catch {} }
