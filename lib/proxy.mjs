...
// DSH remote/LAN workaround: the gate authenticates and rewrites API requests to loopback,
// so the browser-side connection must also expose a host-owned transport. This is intentionally
// injected before client modules are evaluated; patching the served module URL alone is brittle.
const DSH_TRANSPORT_SHIM = `<script data-dsh-gate-transport="1">!function(){try{var t=self.__DSH_TRANSPORT__;if(!t||typeof t!=="object")t={};t.ownsHost=true;self.__DSH_TRANSPORT__=t;}catch(e){}}();</script>`;
export const DEFAULT_INJECT = RANDOM_UUID_POLYFILL + DSH_TRANSPORT_SHIM;

// 退出浮标：注入非管理员条目的 HTML 页（访客/工具间没有设置页，需要 door 上的退出按钮）。
// 点击走 POST /gate/logout（SameSite=Lax 下跨站带不上 cookie，天然防 Logout CSRF；无 JS 时退化为 GET → 405 提示页）。
const LOGOUT_CHIP_HTML = `<style id="dsh-gate-logout-style">#dsh-gate-logout{position:fixed;right:12px;bottom:12px;z-index:2147483647;font:12px/1.4 -apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;padding:7px 12px;border-radius:999px;border:1px solid rgba(128,128,128,.45);background:rgba(127,127,127,.14);color:inherit;opacity:.55;transition:opacity .2s;text-decoration:none;backdrop-filter:blur(4px)}#dsh-gate-logout:hover{opacity:1}</style><a id="dsh-gate-logout" href="/gate/logout" onclick="fetch('/gate/logout',{method:'POST'}).then(function(){location.href='/gate-login'});return false" title="退出密码门会话 | exit gate session" rel="noreferrer">🚪 退出</a>`;
...
// 0.3.7 accepts both legacy and DSH 0.1.5's transport.ownsHost-prefixed expression.
...
// 网关保留 Cookie 命名空间：上游绝不能通过 Set-Cookie 写入这组名字，
// 否则可覆盖浏览器里的门会话 / 设密令牌，造成强制登出或会话混乱。
// 三个出口（透传 / 改写 / WS 握手）统一走 sanitizeUpstreamSetCookie()，新增出口也必须经它。
const RESERVED_COOKIES = new Set([SESSION_COOKIE.toLowerCase(), 'dws_setup']);
export function sanitizeUpstreamSetCookie(headers) {
  const sc = headers?.['set-cookie'];
  if (sc === undefined) return headers;
  const list = Array.isArray(sc) ? sc : [String(sc)];
  const kept = list.filter((c) => {
    const name = String(c).split(';', 1)[0].split('=')[0].trim().toLowerCase();
    return !RESERVED_COOKIES.has(name);
  });
  if (kept.length === list.length) return headers; // 无事发生，保持原样
  if (kept.length === 0) delete headers['set-cookie'];
  else headers['set-cookie'] = Array.isArray(sc) ? kept : kept[0];
  return headers;
}

// 入向转发头清洗：客户端自带的代理断言头一律丢弃，再按 trustProxy 重建规范值——
// trustProxy=false（默认）：只信 socket 直连地址；true：接受可信前代注入的左端值。
// 上游看到的 X-Forwarded-* 永远是门断言的，不再是客户端伪造的。
const INGRESS_PROXY_HEADERS = ['forwarded', 'x-forwarded-for', 'x-forwarded-host', 'x-forwarded-proto', 'x-forwarded-port', 'x-forwarded-ssl', 'x-real-ip', 'true-client-ip', 'cf-connecting-ip'];
export function sanitizeIngressHeaders(headers, req, trustProxy = false) {
  const origHost = headers.host;
  for (const k of INGRESS_PROXY_HEADERS) delete headers[k];
  const sockIp = req.socket?.remoteAddress ?? 'unknown';
  const sockProto = req.socket?.encrypted ? 'https' : 'http';
  if (trustProxy) {
    const xff = String(req.headers['x-forwarded-for'] ?? '').split(',')[0]?.trim();
    headers['x-forwarded-for'] = (xff && xff !== 'unknown') ? xff.slice(0, 64) : sockIp;
    const xfp = String(req.headers['x-forwarded-proto'] ?? '').toLowerCase();
    headers['x-forwarded-proto'] = (xfp === 'https' || xfp === 'http') ? xfp : sockProto;
  } else {
    headers['x-forwarded-for'] = sockIp;
    headers['x-forwarded-proto'] = sockProto;
  }
  if (origHost) headers['x-forwarded-host'] = String(origHost).slice(0, 253);
  return headers;
}
...
export function createGateProxy({ port = 3081, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, upstreams = null, log = null, injectHtml = DEFAULT_INJECT, auth = null, maxLoginAttempts = 3, loginLockMs = 60_000, onAccess = null, trustProxy = false, dshAuth = null, getDshAuth = null, getUpstreams = null, clientHostTrust = true } = {}) {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      // ---- 登出（仅 POST：防 Logout CSRF；设置页按钮与退出浮标都用 POST）----
      if (req.url === '/gate/logout') {
        if (req.method !== 'POST') {
          res.writeHead(405, { 'content-type': 'text/plain; charset=utf-8', 'allow': 'POST', 'cache-control': 'no-store' });
          res.end('dsh-web-pass: 请用 POST 注销（设置页退出按钮 / 页面退出浮标）| logout requires POST');
          return;
        }
      ...
      if (req.url && req.url.startsWith('/dsh-logs/')) {
        ...
        const logHeaders = sanitizeIngressHeaders({ ...req.headers }, req, trustProxy);
  ...
  server.on('upgrade', async (req, socket, head) => {
    ...
    const headers = loopbackAuthority(sanitizeIngressHeaders({ ...req.headers }, req, trustProxy), up);
    ...
    proxyReq.on('upgrade', (proxyRes, proxySocket, proxyHead) => {
      ...
      const raw = [];
      for (const [k, v] of Object.entries(sanitizeUpstreamSetCookie({ ...proxyRes.headers }))) raw.push(`${k}: ${Array.isArray(v) ? v.join(', ') : v}`);
...
function sendRewritten(proxyRes, res, rewrite) {
  collectBody(proxyRes, res, (buf) => {
    ...
    const h = sanitizeUpstreamSetCookie({ ...proxyRes.headers }); // 上游不得写门保留 Cookie
    delete h['content-length']; delete h['transfer-encoding'];
...
async function proxyForward(req, res, { upstream, injectHtml, dshAuth, clientHostTrust = true, logoutChip = false, refreshToken = null, trustProxy = false } = {}) {
  ...
  // 先洗入向断言头（防客户端伪造 X-Forwarded-* 欺骗上游），再改写 authority
  const headers = loopbackAuthority(sanitizeIngressHeaders({ ...req.headers }, req, trustProxy), up);
  ...
  proxyReq = httpRequest({ host: up.host, port: up.port, method: req.method, path: req.url, headers, agent: false, timeout: UPSTREAM_IDLE_MS }, (proxyRes) => {
    ...
    if (clientHostTrust && !isCompressed
      ...
      && (String(req.url ?? '').includes(REWRITE_MODULE_MARK) || String(req.url ?? '').includes('dsh-client-connection'))) {
      sendRewritten(proxyRes, res, (js) => {
        // DSH 0.1.5 在 isLoopbackHostname(...) 前增加了 transport?.ownsHost / pageLocation 判断。
        // 这里直接把完整 isLoopback 表达式替换成 true，确保 Settings 的 host persistence
        // 在经 dsh-web-pass 的可信门后仍可用；服务端 /api trust fence 不被改变。
        const patterns = [
          /transport\?\.ownsHost\s*===\s*true\s*\|\|\s*pageLocation\s*===\s*void 0\s*\|\|\s*isLoopbackHostname\(pageLocation\.hostname\)/,
          /transport\?\.ownsHost\s*\|\|\s*pageLocation\s*===\s*void 0\s*\|\|\s*isLoopbackHostname\(pageLocation\.hostname\)/,
          /pageLocation\s*===\s*void 0\s*\|\|\s*isLoopbackHostname\(pageLocation\.hostname\)/,
          /isLoopbackHostname\(pageLocation\.hostname\)/,
        ];
        let changed = false;
        for (const re of patterns) {
          if (re.test(js)) { js = js.replace(re, 'true'); changed = true; break; }
        }
        if (changed) {
          console.info('dsh-web-pass: 已改写 client-connection isLoopback（兼容 DSH 0.1.5 remote settings）');
        } else {
          console.warn('dsh-web-pass: 未找到 isLoopback 表达式，模块原样透传（DSH 版本可能已更新）');
        }
        return js;
      });
      return;
    }
    // 响应原样透传（压缩交给前端反向代理做；门保留 Cookie 同样不得由上游写入）
    res.writeHead(proxyRes.statusCode ?? 502, sanitizeUpstreamSetCookie({ ...proxyRes.headers }));
    proxyRes.pipe(res);
