...
export function createGateProxy({ port = 3081, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT, auth = null, maxLoginAttempts = 3, loginLockMs = 60_000, onAccess = null, trustProxy = false, dshAuth = null, clientHostTrust = true } = {}) {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      if (!auth) { await proxyForward(req, res, upstream, injectHtml, dshAuth, clientHostTrust); return; }
      ...
      // ---- 已认证：代理到上游 ----
      await proxyForward(req, res, upstream, injectHtml, dshAuth, clientHostTrust);
    } catch (e) {
...
async function proxyForward(req, res, upstream, injectHtml, dshAuth, clientHostTrust) {
  const headers = loopbackAuthority({ ...req.headers }, upstream);
