...
import { escapeHtml } from './gate-pages.mjs'; // 查看器转义与门页面共用同一份实现
...
const LINE_RE = /^(?<ip>\S+) - - \[(?<time>[^\]]+)\] "(?<request>[^"]*)" (?<status>\d{3}) (?<bytes>\S+) "(?<referer>[^"]*)" "(?<ua>[^"]*)"(?<label> "[^"]*")?$/;
...
/** 构造 nginx-combined 风格的一行访问日志（IP 已假名化；label 为上游条目名，可空——空时行格式与旧版逐字节一致）。 */
export function buildAccessLine(req, res, trustProxy = false, label = '') {
  ...
  const tag = label ? ` "${String(label).replace(/[\r\n"]/g, '')}"` : '';
  return `${ip} - - [${time}] "${request}" ${status} ${bytes} "${referer}" "${ua}"${tag}`;
}
...
export function createAccessLogger({ file, maxBytes = DEFAULT_LOG_MAX_BYTES, maxFiles = DEFAULT_LOG_MAX_FILES, trustProxy = false }) {
  ...
  void ensure();
  /** @param {import('node:http').IncomingMessage} req @param {import('node:http').ServerResponse} res @param {string} label 上游条目名（可空） */
  return {
    async log(req, res, label = '') {
      const line = buildAccessLine(req, res, trustProxy, label);
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      for (let l = lines.length - 1; l >= 0; l--) { // 最新在上
        ...
        if (!m) continue;
        const g = m.groups;
        const entryLabel = (g.label ?? '').trim().replace(/^"|"$/g, '');
        if (filter && !(`${g.ip} ${g.request} ${entryLabel}`.toLowerCase().includes(filter))) continue;
        const st = statusLabel(g.status);
        const t = parseNginxTime(g.time); // 文件存 UTC，展示转服务器本地时区
        const timeShow = t ? localTimeStr(t) : g.time;
        rows.push(
          `<tr><td>${escapeHtml(g.ip)}</td><td>${escapeHtml(timeShow)}</td><td class="req">${escapeHtml(g.request)}</td>` +
          `<td class="${st.cls}">${st.txt}${st.code ? ` <span style="opacity:.5">${st.code}</span>` : ''}</td>` +
          `<td>${escapeHtml(entryLabel || '-')}</td>` +
          `<td>${escapeHtml(g.bytes)}</td><td class="small">${escapeHtml(g.referer)}</td><td class="small">${escapeHtml(g.ua)}</td></tr>`
        );
      }
...
<form method="get" action="/"><input name="ip" placeholder="按 IP / 请求筛选" value="${escapeHtml(u.searchParams.get('ip') ?? '')}">
<input name="limit" type="number" min="1" max="5000" value="${want}" style="width:110px"><button type="submit">筛选</button></form>
</header><main><table><thead><tr><th>IP</th><th>时间</th><th>请求</th><th>验证结果</th><th>入口</th><th>大小</th><th>来源</th><th>UA</th></tr></thead>
<tbody>${rows.join('') || '<tr><td colspan="8" style="color:#9fb3c8">暂无记录 | no records yet</td></tr>'}</tbody></table></main></body></html>`;
