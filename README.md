...
- **多密码多上游**（v0.3.2 新增）：一个密码进一个后端——主人密码进老 DSH，访客密码进干净 DSH，还能再挂 openclaw 等本地服务。登录还是同一个密码框，后端看对上哪条；报错永远笼统，不透露哪条。
- **设置页直接加上游**（v0.3.3 新增）：上游表里填「标签 + 上游」点「增加」即可，新条目默认停用、无密码；存数据目录 `upstreams.json`，重启保留，patch 行照旧可用。
...
| `upstreams` | `[]` | 多密码多上游表（见下文“多密码多上游”）：每条含 `label` / `passwordEnv` / `host` / `port` / `clientHostTrust` / `dsh` / `enabled`；不配即单密码行为 |

> **关于 `trustProxy`**：默认关闭时，访客 IP 与登录限速只依据 socket 直连地址，防止伪造 XFF 头污染日志或绕过限速。仅当你在网关前面部署了可信的反向代理（nginx、Cloudflare 隧道等）时才开启。

## 多密码多上游（v0.3.2 新增：访客模式）

```
浏览器 → dsh-web-pass :3081 → 按密码分流 → 127.0.0.1:3080（主人老 DSH）
                                          → 127.0.0.1:3085（访客干净 DSH）
                                          → 127.0.0.1:5101（openclaw 等本地服务）
```

- **加一行=加一个密码+一个后端**：优先在设置页「上游表」里填**标签 + 上游（host:port）**点「增加」（v0.3.3 新增，即时生效、无需重起；新行**默认停用、无密码**）；也可改 `cordis.patch.yml` 的 `upstreams` 加一条后**重起** `dsh web`。页面上新增的行保存在 `$DSH_HOME/dsh-web-pass/upstreams.json`（0600，重启保留），与 patch 行共存、追加在其后；密码值可在设置页「网页密码」活改，不用重起。
- **访客密码只能主人设**：没有注册页。设置页上游表里点对应行的「密码重设」，或下方统一框选条目再设（条目下拉点表行自动选中）。保存后该条旧密码立即作废、其会话全部吊销；改密码不用输原密码（登录态即凭证）。
...
- **DSH 类后端才代持/改写**：`dsh: true` 的条目各持一份 DSH cookie 并可改写模块；反代 openclaw 等非 DSH 请设 `dsh: false`（默认），原样透传，免得把 DSH cookie 带过去。
- **管理面只在主人间**：设置页（含上游表、日志入口 `/dsh-logs/`）只存在于装了本插件的主 DSH；访客干净间不装本插件，访客碰不到管理面。停用某条即刻踢掉其会话（无需重起）；删除某条即刻失效（软删：行隐藏、序号不变），`patch` 里删行重起则彻底消失；管理员条目不可停用、不可删除。
...
- **哪里都能退出**（v0.3.3）：访客/工具间没有设置页——其页面右下角自动出现「🚪 退出」浮标（半透明小按钮，不遮内容），点击即清会话回登录页；也可直接访问 `/gate/logout`（GET/POST 皆可）。主人间设置页照旧自带退出按钮。
...
- 每行末尾附条目名（如访客进的记 `访客`；v0.3.2 前的老行无该列，查看器显示 `-`），支持按条目筛选。
