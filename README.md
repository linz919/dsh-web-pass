...
- **登录同一个框**：输错统一报「密码错误，请重试」，不透露是哪条；限速各条共用计数器。某条后端没起来，该条登录提示「该入口暂未启用」，主人照常用。
...
- **条目序号 = 会话绑定键**：会话按条目在表中的位置（序号）绑定，顺序为「管理员 → patch 行 → 页面新增行」。只追加、软删保序；若手工调整 patch 里 `upstreams` 行序，重起前先清空同目录 `sessions.jsonl`，否则旧会话（≤2 天）可能映射到另一条后端。
